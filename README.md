...
- 🖼️ **自動画像最適化** - Cloudflare Image Resizingで読み込み時に自動最適化
- 🚀 **真の遅延読み込み** - 画面に表示される直前に画像を読み込み
- 📸 **自動画像検出** - `manifest.json` 付きのフォルダならファイル名は自由
- 🐍 **Python + Pillow 最適化** - 画像圧縮と manifest 生成を一括処理
...
# 任意のファイル名でもOK
# ただし manifest.json を更新してください
```

詳しくは **[PHOTO_MANAGEMENT.md](PHOTO_MANAGEMENT.md)** をご覧ください。
...
### 🎯 自動画像検出
- フォルダに画像を入れるだけで自動表示
- ファイル名を設定ファイルに記述する必要なし
- 対応パターン: `1.jpg`, `photo1.jpg`, `IMG_0001.jpg` などの連番も、`manifest.json` があれば任意名も対応
- `optimize-images.sh` で最適化と manifest 生成を一括実行

### ⚡ 自動最適化
- 読み込み時に自動的に最適なサイズに変換
- デバイスの画面サイズに応じて最適化
- Cloudflare Image Resizing対応（要設定）
...
詳しくは:
- **[AUTOMATIC_IMAGE_OPTIMIZATION.md](AUTOMATIC_IMAGE_OPTIMIZATION.md)** - 自動最適化ガイド
- **[IMAGE_OPTIMIZATION_GUIDE.md](IMAGE_OPTIMIZATION_GUIDE.md)** - 手動最適化ガイド
- **[PERFORMANCE_IMPROVEMENTS.md](PERFORMANCE_IMPROVEMENTS.md)** - 技術詳細
...
> **💡 画像管理がとても簡単！**  
> カテゴリーフォルダに画像を入れて、`optimize-images.sh` を実行すると `manifest.json` が更新され、自動的に表示されます。
