...
# バックアップなしで最適化（すでに元画像を別の場所に保存している場合）
./optimize-images.sh --no-backup

# 古いバックアップを手動で削除
python3 optimize-images.py cleanup img/photos

# バックアップを1件だけ残して削除
python3 optimize-images.py cleanup img/photos --keep 1
```

スクリプトは自動的に:
- 元の画像をバックアップ（**最新1世代のみ保持**、古いバックアップは自動削除）
- 1500x1500px以下にリサイズ
- 品質85%で圧縮
- 削減量を表示

> **💡 バックアップについて:** 以前のバージョンでは実行のたびにバックアップが
> 無限に蓄積されていましたが、現在は最新1世代のみ保持されます。
> バックアップが不要な場合は `--no-backup` オプションを使用してください。
...
### Q: 間違って圧縮しすぎた
A: バックアップから復元してください（最新1世代が自動保持されています）

### Q: バックアップフォルダが溜まっている
A: `python3 optimize-images.py cleanup img/photos` で一括削除できます。
   現在のバージョンでは自動で最新1件のみ保持されるため、通常は手動削除は不要です。
